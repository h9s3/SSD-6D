...
def load_bbx(path):
    with open(path, 'r') as f:
        gtsbbx = yaml.load(f, Loader=yaml.CLoader)
        for im_id, gtsbbx_im in gtsbbx.items():
            for gtbbx in gtsbbx_im:
                if 'bbox_obj' in gtbbx.keys():
                    gtbbx['bbox_obj'] = np.array(gtbbx['bbox_obj'])
    return gtsbbx
...
def load_sixd(base_path, seq, nr_frames=0, load_mesh=True):
    ...
    path = os.path.join(base_path, 'test/{:06d}/'.format(seq))
    info = load_info(os.path.join(path, 'scene_camera.yml'))
    gts = load_gt(os.path.join(path, 'scene_gt.yml'))
    gtsbbx = load_bbx(os.path.join(path, 'scene_gt_info.yml'))
    for i in range(len(gts)):
        for gt in gts[str(i)]:
            for bbx in gtsbbx[str(i)]:
                gt["obj_bb"] = bbx["bbox_obj"]
    ...
    for i in range(nr_frames):
        ...
        if os.path.exists(path + 'mask'):
            fr.mask = cv2.imread(path + 'mask/' + nr_string +"_"+"000000"+ ".png", -1)

        for gt in gts[str(i)]:
            pose = np.identity(4)
            pose[:3, :3] = gt['cam_R_m2c']
            pose[:3, 1] = np.squeeze(gt['cam_t_m2c']) * bench.scale_to_meters
            fr.gt.append((gt['obj_id'], pose, gt['obj_bb']))

        fr.cam = info[str(i)]['cam_K']

        if bench.cam is None:
            bench.cam = info[str(i)]['cam_K']
